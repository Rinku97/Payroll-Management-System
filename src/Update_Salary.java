...
// Parameters:x the new x-coordinate of this componenty, the new y-coordinate of this component, width the new width of this component, height the new height of this component		
		...
		choice.setBounds(120,20,50,20);
		add(choice);
		
		JLabel name = new JLabel("Emp Name");
		t6 = new JTextField(15);
		t6.setBounds(240, 20,130,20);
		name.setBounds(175, 20, 200, 20);
		add(name);
		add(t6);
	...
	public void itemStateChanged(ItemEvent e) {
		...
		ResultSet name;
		try {
			name = conn.s.executeQuery("select * from employee where id="+choice.getSelectedItem());
			if(name.next())
			{
				t6.setText(name.getString("name"));
			}
